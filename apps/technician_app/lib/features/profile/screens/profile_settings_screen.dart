...
class _ProfileSettingsScreenState extends State<ProfileSettingsScreen> {
  ...
  final List<Map<String, String>> _loginHistory = [
    {
      ...
      'time': 'Hôm nay - 08:12',
      'ip': '14.232.12.85 (Hà Nội)',
    },
    {
      'device': 'Samsung Galaxy S24 Ultra • Android 14',
      'time': '25/05/2026 - 13:45',
      'ip': '113.190.45.122 (Hà Nội)',
    },
    {
      'device': 'iPad Pro • iPadOS 17.2',
      'time': '20/05/2026 - 09:10',
      'ip': '27.72.90.15 (Hà Nội)',
    },
  ];
  ...
  void _handleAvatarUpdate() {
    final hasAvatar =
        context.read<AuthController>().currentUser?.avatar != null;
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Đã hủy liên kết tất cả các thiết bị khác!'),
                    backgroundColor: Color(0xff10b981),
                  ),
                );
              },
                  ...
                  onPressed: () async {
                    ...
                    if (success) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            profileController.error ?? 'Lỗi không xác định',
                          ),
                          backgroundColor: AppColors.error,
                          behavior: SnackBarBehavior.floating,
                        ),
  ...
  Widget _buildSpecializationsCard(dynamic user) {
    final List<String> specs =
        (user?.specializations as List<dynamic>?)
            ?.map((e) => e.toString())
            .toList() ??
        [];
      ...
      child: Column(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            subtitle: const Text(
              'Báo lịch phân công, thay đổi lịch hẹn nhanh',
              style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.bold,
                color: Color(0xff94a3b8),
          ...
          SwitchListTile(
            contentPadding: EdgeInsets.zero,
            title: const Text(
              'Thông báo SMS khẩn cấp',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14),
            ),
            subtitle: const Text(
              'Tự động gửi tin nhắn SMS khi được điều chuyển ca lắp khẩn',
              style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.bold,
                color: Color(0xff94a3b8),
